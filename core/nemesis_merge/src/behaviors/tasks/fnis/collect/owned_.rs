            ...
            TARGET_PATTERNS
                ...
                .flat_map(|pattern| {
                    ...
                    collect_paths(&glob_pat)
                        ...
                        .filter_map(|ns_path| {
                            let ns_name = ns_path.file_name()?.to_string_lossy();
                            let priority = fnis_entries.get(ns_name.as_ref()).copied()?;

                            match collect_fnis_injection(&ns_path, priority) {
                                Ok(injection) => Some(injection),
                                Err(e) => {
                                    tracing::warn!(namespace=%ns_name, error=%e, "Failed to collect FNIS injection");
                                    None
                                }
                            }
                        })
