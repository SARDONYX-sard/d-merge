...
use self::oar_json::{prepare_anim_config_json, FnisToOarConfig};

use crate::behaviors::tasks::fnis::collect::owned::OwnedFnisInjection;
use crate::behaviors::tasks::fnis::list_parser::patterns::alt_anim::AlternativeAnimation;
use crate::behaviors::tasks::fnis::patch_gen::hkx_convert::{AnimIoJob, ConversionJob};
use crate::behaviors::tasks::templates::generated_group_table::ALT_GROUPS;
...
pub fn alt_anim_to_oar(
    ...
    for set in &alt_anim.set {
        ...
        let Some(group) = ALT_GROUPS.get(group_name) else {
            errors.push(Error::Custom {
