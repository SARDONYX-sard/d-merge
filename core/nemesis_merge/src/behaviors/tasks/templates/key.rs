...
impl<'a> TemplateKey<'a> {
    ...
    pub const unsafe fn new_unchecked(template_name: &'a str) -> Self {
        Self { template_name }
    }
...
pub const THIRD_PERSON_0_MASTER: TemplateKey =
    unsafe { TemplateKey::new_unchecked("meshes/actors/character/behaviors/0_master.bin") };
...
impl core::fmt::Debug for TemplateKey<'_> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        core::fmt::Debug::fmt(Path::new(self.template_name), f)
    }
}
...
impl core::fmt::Display for TemplateKey<'_> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "{}", Path::new(self.template_name).display())
    }
}
...
impl<'a, 'b> PartialEq<TemplateKey<'b>> for TemplateKey<'a> {
    fn eq(&self, other: &TemplateKey<'b>) -> bool {
        Path::new(self.template_name) == Path::new(other.template_name)
    }
}

impl<'a> PartialEq<Path> for TemplateKey<'a> {
    fn eq(&self, other: &Path) -> bool {
        Path::new(self.template_name) == other
    }
}
...
impl<'a, 'b> PartialOrd<TemplateKey<'b>> for TemplateKey<'a> {
    fn partial_cmp(&self, other: &TemplateKey<'b>) -> Option<core::cmp::Ordering> {
        Path::new(self.template_name).partial_cmp(Path::new(other.template_name))
    }
}

impl<'a> Ord for TemplateKey<'a> {
    fn cmp(&self, other: &Self) -> core::cmp::Ordering {
        Path::new(self.template_name).cmp(Path::new(other.template_name))
    }
}
...
impl<'a> core::hash::Hash for TemplateKey<'a> {
    fn hash<H: core::hash::Hasher>(&self, state: &mut H) {
        Path::new(self.template_name).hash(state);
    }
}
...
/// Nemesis 1st person to meshes rel template .bin path
#[rustfmt::skip]
static FNIS_INJECT_TARGET_1ST_PERSON_MAP: phf::Map<&'static str, &'static str> = phf::phf_map! {
    "0_master" => "meshes/actors/character/_1stperson/behaviors/0_master.bin",
};

/// Nemesis third person to meshes rel template .bin path
#[rustfmt::skip]
static FNIS_INJECT_TARGET_3RD_PERSON_MAP: phf::Map<&'static str, &'static str> = phf::phf_map! {
    "0_master" => "meshes/actors/character/behaviors/0_master.bin",
    "AttackBehavior" => "meshes/actors/character/behaviors/AttackBehavior.bin",
    "atronachflamebehavior" => "meshes/actors/atronachflame/behaviors/atronachflamebehavior.bin",
    "atronachfrostbehavior" => "meshes/actors/atronachfrost/behaviors/atronachfrostbehavior.bin",
