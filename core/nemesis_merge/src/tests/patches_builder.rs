...
pub(crate) fn build_patch_maps(config: PatchMapsConfig<'_>) -> PatchMaps {
    ...
    let (nemesis_entries, fnis_groups): (PriorityMap, Vec<Vec<(String, usize)>>) = mod_list
        .par_iter()
        .enumerate()
        .filter_map(|(idx, info)| {
            ...
            match info.mod_type {
                ...
                ModType::Fnis if !fnis_excludes.contains(mod_id) => Some(Either::Right(
                    info.fnis_list_paths.iter().map(|p| (p.display().to_string(), idx)).collect(),
                )),
