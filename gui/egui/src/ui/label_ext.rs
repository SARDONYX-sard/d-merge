...
pub(crate) fn label_with_hover(ui: &mut Ui, text: &str, width: f32) {
    ...
    let display =
        if truncated.is_empty() { " ".repeat((width / 6.0).max(3.0) as usize) } else { truncated };
    ui.add_sized([width, ROW_HEIGHT], egui::Label::new(display).halign(Align::LEFT))
        .on_hover_text(text);
}
...
pub(crate) fn hyperlink_with_hover(ui: &mut Ui, url: &str, width: f32) {
    ...
    ui.add_sized([width, ROW_HEIGHT], egui::Hyperlink::from_label_and_url(truncated, url))
        .on_hover_text(url);
}
...
fn truncate_to_width(ui: &Ui, text: &str, width: f32) -> String {
    ...
    let galley_x_size = ui.fonts_mut(|font| {
        font.layout_no_wrap(text.to_string(), font_id.clone(), text_color).size().x
    });
    ...
    for ch in text.chars() {
        let galley_x_size = ui.fonts_mut(|font| {
            font.layout_no_wrap(format!("{truncated}{ch}..."), font_id.clone(), text_color).size().x
        });
